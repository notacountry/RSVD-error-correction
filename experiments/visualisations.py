...
def generate_results(gen, n, k, p, seed):
    """
    Sample one matrix from gen and run plain + corrected RSVD.
    ...
    dict with keys:
        ...
        Sigma_corr  : (k,) corrected RSVD singular values
    ...
    _, Sigma_corr, _ = rsvd(A, k, p=p, seed=seed, correction=True)
    _, eigs_corr, _ = sketch_spectral_info(Y, m, n_cols, l, k)
    return dict(sigma_true=sigma_true, Sigma_rsvd=Sigma_rsvd, Sigma_corr=Sigma_corr, eigs_corr=eigs_corr)


def plot_generator(name, results):
    """
    1×2 figure for one generator: eigenvalue spectrum (left) and singular value
    estimates vs true (right).
    """
    ...
    _plot_singular_values(ax2, sigma_true, Sigma_corr, Sigma_rsvd, k)
...
def _plot_singular_values(ax, sigma_true, Sigma_corr, Sigma_rsvd, k):
